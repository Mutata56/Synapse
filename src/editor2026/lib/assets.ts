...
// под текущую машину при загрузке/отрисовке. Имя ассета — это sha256+расширение,
 ...
 * тоже отдаёт исходную строку — лучше показать ничего, чем уронить рендер.
 */
...
/** Сворачивает к портабельной форме ссылки в одной gallery (проп `items` —
 *  это JSON-строка `[{url,name,caption}]`). Возвращает ту же строку, если
 ...
 *  поменяли — отдаём поверхностный клон, где нет — исходный объект. */
